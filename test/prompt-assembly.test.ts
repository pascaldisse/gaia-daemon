...
import type { MemoryState } from "../src/memory/memory-store.ts";
...
test("system prompt assembles persona, role, project intent, context, and memory in order", () => {
...
test("turn prompt uses new room events instead of whole transcript wording", () => {
  const prompt = buildTurnPrompt({
    ...
    events: [
      { timestamp: "2026-05-02T00:00:00.000Z", author: "user", targets: ["gaia"], text: "Newest question" },
    ],
  });
