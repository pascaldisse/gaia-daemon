...
function syncGlobalSettingsSelection() {
...
function renderJsonFields(container, value, path = []) {
  ...
  for (const [key, entryValue] of entries) {
    ...
    if (isStructuredJsonValue(entryValue)) {
      ...
      renderJsonFields(nested, entryValue, entryPath);
...
function serializeSettings(view, kind) {
  if (kind === "json") {
    ...
    for (const input of view.querySelectorAll("[data-json-path]")) {
      setJsonPathValue(next, JSON.parse(input.dataset.jsonPath || "[]"), parseJsonFieldValue(input.value));
    }
