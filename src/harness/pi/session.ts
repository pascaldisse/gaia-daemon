// Pi runtime internals; registration data remains in ../pi.ts.
import { homedir } from "node:os";
import { existsSync, readdirSync, readFileSync } from "node:fs";
import { join } from "node:path";
import type { Model } from "@earendil-works/pi-ai";
import { compact as generatePiCompaction, createAgentSession, DefaultResourceLoader, getAgentDir, ModelRegistry, ModelRuntime, SessionManager, SettingsManager } from "@earendil-works/pi-coding-agent";
...
import { NO_SESSION_TO_COMPACT, type AgentDef, type AgentEvent, type CompactResult, type MessageAttachment, type ThinkingLevel, type Workspace } from "../../core/types.js";
...
export function hasPersistedPiSession(rootDir: string, roomId: string, agentId: string): boolean {
  try {
...
export class PiRuntime implements AgentRuntime {
  ...
  async *send(input: AgentInput): AsyncIterable<AgentEvent> {
    ...
    const unsubscribe = session.subscribe((event) => forwardPiEvent(event, session, channel));
