...
function runSidecar(config: Required<Pick<TrinityConfig, "python" | "script">> & TrinityConfig, transcript: string): Promise<RouterReply> {
  return new Promise((resolve, reject) => {
    const args = [
      ...
      ...(config.baseModel ? ["--base-model", config.baseModel] : []),
      ...(config.roles === "off" ? ["--no-roles"] : []),
    ];
...
class TrinityHeadPolicy implements RoutingPolicy {
  ...
  async next(obs: MonadObservation, ctx: RoutingPolicyContext): Promise<MonadOutcome> {
    ...
    const reply = await runSidecar(
      { python: this.config.python ?? "python3", script: this.config.script, weights: this.config.weights, baseModel: this.config.baseModel, roles: this.config.roles },
      renderTranscript(obs.query, obs.steps),
    );
