// The agent-runner subprocess. The daemon spawns exactly this for EVERY harness
// (see RunnerHost) — Pi included, no longer privileged in-process — so execution
// is uniform and the sandbox has one process to wrap. It builds the real runtime
// via the harness registry (its translation code untouched) with bridge-backed
// deps, then runs a single-flight loop: read a command on stdin, stream the
// turn's AgentEvents back on stdout as protocol JSON.
//
// stdout carries ONLY protocol lines, so every console.* is redirected to stderr
...
import "./index.js"; // self-register every harness before the lookup
