...
import { routingPolicySpecFor } from "../runtime/monad/policy-registry.js";
import type {
  ChatMessage,
  MonadConfig,
  MonadObservation,
...
} from "../runtime/monad/types.js";
import { renderMessages, renderTranscript, replyAccepts } from "../runtime/monad/util.js";
...
export class MonadEngine {
  ...
  async run(query: string, runOptions: MonadRunOptions = {}): Promise<MonadResult> {
    ...
    const obs: MonadObservation = { query, steps: [], transcript: renderTranscript(query, []) };

    for (let turn = 0; turn < this.config.maxTurns; turn++) {
      if (isCancelled()) return this.finalize(obs, "cancelled");

      const outcome = await policy.next(obs, this.context());
      if (outcome.kind === "accept") return this.finalize(obs, this.config.terminate?.on ?? "accept", outcome.finalStepIndex);
      if (outcome.kind === "stop") return this.finalize(obs, outcome.reason);
      ...
      obs.steps.push(step);
      obs.transcript = renderTranscript(query, obs.steps);
      ...
      if (this.config.terminate?.on === "verifier-accept" && step.role === "verifier" && replyAccepts(reply, this.config.terminate.acceptToken)) {
        const lastWorker = this.lastWorkerIndex(obs);
        return this.finalize(obs, "verifier-accept", lastWorker ?? step.index);
      }
    }
  ...
  private finalize(obs: MonadObservation, terminatedBy: string, finalStepIndex?: number): MonadResult {
    let index = finalStepIndex;
    if (index === undefined) index = this.lastWorkerIndex(obs) ?? (obs.steps.length > 0 ? obs.steps[obs.steps.length - 1].index : -1);
