...
| Plugin-Mechanismus (Daemon) | ⚠ STALE (09-05, Adversary-Review axis 6): war zwei Systeme, ist jetzt eines. `src/services/plugin-host.ts` existiert nicht mehr. `src/services/plugins.ts` (75 Zeilen) ist nur noch ein Kompatibilitäts-Adapter über `src/services/plugins/{contracts,registry,loader,manifest}.ts` — das reale Manifest-Plugin-System: `contracts.ts` typisierte Contribution-Ports (command/tool/channel-bridge/provider), `registry.ts` staged-reload + Turn-Lease + Disposer, `manifest.ts` `plugin.json`-Discovery unter `~/.gaia/plugins`. `PluginPanel` bleibt **data-only, explizit KEINE iframes** (Kommentar unverändert). Pi-Erweiterungen (registerCommand/registerShortcut/ctx.ui.\*) sind ein SEPARATER, paralleler Pfad (`src/harness/pi/*`) — seit Lane E (chat-mto9n58s-bjr1) erreichbar+event-vollständig, s. `docs/PLUGIN-FORMAT.md`. | `src/services/plugins.ts`, `src/services/plugins/*.ts` |
